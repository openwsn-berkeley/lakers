...
pub fn edhoc_exporter<Crypto: CryptoTrait>(
    ...
    if current_state == EDHOCState::Completed {
        output = crypto.edhoc_kdf(&prk_exporter, label, context, context_len, length);
        Ok((state, output))
    } else {
...
pub fn edhoc_key_update<Crypto: CryptoTrait>(
    ...
    if current_state == EDHOCState::Completed {
        // new PRK_out
        prk_new_buf = crypto.edhoc_kdf(&prk_out, 11u8, context, context_len, SHA256_DIGEST_LEN);
        prk_out[..SHA256_DIGEST_LEN].copy_from_slice(&prk_new_buf[..SHA256_DIGEST_LEN]);

        // new PRK_exporter
        prk_new_buf = crypto.edhoc_kdf(
            &prk_out,
            10u8,
...
pub fn r_prepare_message_2<Crypto: CryptoTrait>(
    ...
    if current_state == EDHOCState::ProcessedMessage1 {
        // compute TH_2
        let th_2 = crypto.compute_th_2(&g_y, &h_message_1);

        // compute prk_3e2m
        let prk_2e = crypto.compute_prk_2e(&y, &g_x, &th_2);
        let salt_3e2m = crypto.compute_salt_3e2m(&prk_2e, &th_2);
        prk_3e2m = crypto.compute_prk_3e2m(&salt_3e2m, r, &g_x);

        // compute MAC_2
        let mac_2 = crypto.compute_mac_2(&prk_3e2m, id_cred_r, cred_r, &th_2);
        ...
        th_3 = crypto.compute_th_3(&th_2, &plaintext_2, cred_r);
        ...
        let (ciphertext_2, ciphertext_2_len) =
            crypto.encrypt_decrypt_ciphertext_2(&prk_2e, &th_2, &ct);
...
pub fn r_process_message_3<Crypto: CryptoTrait>(
    mut state: State,
    crypto: &mut Crypto,
    ...
    if current_state == EDHOCState::WaitMessage3 {
        let plaintext_3 = crypto.decrypt_message_3(&prk_3e2m, &th_3, message_3);
            ...
            if decoded_p3_res.is_ok() {
                ...
                if ead_success {
                    ...
                    if kid == id_cred_i_expected[id_cred_i_expected.len() - 1] {
                        // compute salt_4e3m
                        let salt_4e3m = crypto.compute_salt_4e3m(&prk_3e2m, &th_3);
                        // TODO compute prk_4e3m
                        prk_4e3m = crypto.compute_prk_4e3m(&salt_4e3m, &y, g_i);

                        // compute mac_3
                        let expected_mac_3 = crypto.compute_mac_3(
                            &prk_4e3m,
                            &th_3,
                            id_cred_i_expected,
                            cred_i_expected,
                        ...
                        if mac_3 == expected_mac_3 {
                            ...
                            let prk_out_buf = crypto.edhoc_kdf(
                                &prk_4e3m,
                                7u8,
                                &th_4_buf,
                                th_4.len(),
                            ...
                            let prk_exporter_buf = crypto.edhoc_kdf(
                                &prk_out,
                                10u8,
...
// returns c_r
pub fn i_process_message_2<Crypto: CryptoTrait>(
    mut state: State,
    crypto: &mut Crypto,
    ...
    if current_state == EDHOCState::WaitMessage2 {
        ...
        if res.is_ok() {
            ...
            // compute prk_2e
            let prk_2e = crypto.compute_prk_2e(&x, &g_y, &th_2);

            let (plaintext_2, plaintext_2_len) =
                crypto.encrypt_decrypt_ciphertext_2(&prk_2e, &th_2, &ciphertext_2);
            ...
            if plaintext_2_decoded.is_ok() {
                ...
                if ead_success {
                    ...
                    prk_3e2m = crypto.compute_prk_3e2m(&salt_3e2m, &x, g_r);

                    let expected_mac_2 =
                        crypto.compute_mac_2(&prk_3e2m, id_cred_r_expected, cred_r_expected, &th_2);
                    ...
                    if mac_2 == expected_mac_2 {
                        if kid == id_cred_r_expected[id_cred_r_expected.len() - 1] {
                            ...
                            th_3 = crypto.compute_th_3(&th_2, &pt2, cred_r_expected);
                            // message 3 processing

                            let salt_4e3m = crypto.compute_salt_4e3m(&prk_3e2m, &th_3);

                            prk_4e3m = crypto.compute_prk_4e3m(&salt_4e3m, i, &g_y);
...
pub fn i_prepare_message_3<Crypto: CryptoTrait>(
    ...
    if current_state == EDHOCState::ProcessedMessage2 {
        let mac_3 = crypto.compute_mac_3(&prk_4e3m, &th_3, id_cred_i, cred_i);

        let ead_3 = i_prepare_ead_3();

        let plaintext_3 = encode_plaintext_3(id_cred_i, &mac_3, &ead_3);
        message_3 = crypto.encrypt_message_3(&prk_3e2m, &th_3, &plaintext_3);

        let th_4 = crypto.compute_th_4(&th_3, &plaintext_3, cred_i);
        ...
        // PRK_out = EDHOC-KDF( PRK_4e3m, 7, TH_4, hash_length )
        let prk_out_buf =
            crypto.edhoc_kdf(&prk_4e3m, 7u8, &th_4_buf, th_4.len(), SHA256_DIGEST_LEN);
...
// All these functions that are generally pure functions without any state they handle are
// expressed as an extension to the Crypto trait, because this gives the easiest invocation
// semantics -- any code outside them can just call `crypto.compute_th_2()`, code inside calls
// `self.some_function()`.
//
// It's easier to write because it allows the Crypto generics to sit on the trait instead of being
// repeated for each function. (Other than that, they could all just as well be called
// `computed_th_2(crypto, ...)` etc).
trait CryptoExt: CryptoTrait {
    fn compute_th_2(&mut self, g_y: &BytesP256ElemLen, h_message_1: &BytesHashLen) -> BytesHashLen {
        ...
        let th_2 = self.sha256_digest(&message, len);
    ...
    fn compute_th_3(
        ...
        let output = self.sha256_digest(&message, th_2.len() + 2 + plaintext_2.len + cred_r.len());
    ...
    fn compute_th_4(
        ...
        let output = self.sha256_digest(&message, th_3.len() + 2 + plaintext_3.len + cred_i.len());
    ...
    fn edhoc_kdf(
        ...
        let output = self.hkdf_expand(prk, &info, info_len, length);
    ...
    fn compute_k_3_iv_3(
        ...
        let k_3_buf = self.edhoc_kdf(prk_3e2m, 3u8, &th_3_buf, th_3.len(), AES_CCM_KEY_LEN);
        ...
        let iv_3_buf = self.edhoc_kdf(prk_3e2m, 4u8, &th_3_buf, th_3.len(), AES_CCM_IV_LEN);
    ...
    fn encrypt_message_3(
        ...
        let (k_3, iv_3) = self.compute_k_3_iv_3(prk_3e2m, th_3);

        let ciphertext_3 = self.aes_ccm_encrypt_tag_8(&k_3, &iv_3, &enc_structure, plaintext_3);
    ...
    fn decrypt_message_3(
        ...
        let p3 = self.aes_ccm_decrypt_tag_8(&k_3, &iv_3, &enc_structure, &ciphertext_3);
    ...
    fn compute_mac_2(
        ...
        mac_2[..].copy_from_slice(
            &self.edhoc_kdf(prk_3e2m, 2 as u8, &context, context_len, MAC_LENGTH_2)[..MAC_LENGTH_2],
        );
    ...
    fn encrypt_decrypt_ciphertext_2(
        ...
        let keystream_2 = self.edhoc_kdf(
            prk_2e,
            0u8,
            &th_2_context,
            SHA256_DIGEST_LEN,
    ...
    fn compute_salt_4e3m(&mut self, prk_3e2m: &BytesHashLen, th_3: &BytesHashLen) -> BytesHashLen {
        let mut th_3_context: BytesMaxContextBuffer = [0x00; MAX_KDF_CONTEXT_LEN];
        th_3_context[..th_3.len()].copy_from_slice(&th_3[..]);
        let salt_4e3m_buf =
            self.edhoc_kdf(prk_3e2m, 5u8, &th_3_context, th_3.len(), SHA256_DIGEST_LEN);
    ...
    fn compute_prk_4e3m(
        ...
        let g_iy = self.p256_ecdh(i, g_y);
        let prk_4e3m = self.hkdf_extract(salt_4e3m, &g_iy);

        prk_4e3m
    }

    fn compute_salt_3e2m(&mut self, prk_2e: &BytesHashLen, th_2: &BytesHashLen) -> BytesHashLen {
        let mut th_2_context: BytesMaxContextBuffer = [0x00; MAX_KDF_CONTEXT_LEN];
        th_2_context[..th_2.len()].copy_from_slice(&th_2[..]);

        let salt_3e2m_buf = self.edhoc_kdf(
            prk_2e,
            1u8,
            &th_2_context,
            SHA256_DIGEST_LEN,
    ...
    fn compute_prk_3e2m(
        ...
        // compute g_rx from static R's public key and private ephemeral key
        let g_rx = self.p256_ecdh(x, g_r);
        let prk_3e2m = self.hkdf_extract(salt_3e2m, &g_rx);
...
impl<Crypto: CryptoTrait> CryptoExt for Crypto {}
...
mod tests {
    ...
    fn test_compute_th_2() {
        let th_2 = default_crypto().compute_th_2(&G_Y_TV, &H_MESSAGE_1_TV);
    ...
    fn test_compute_th_3() {
        ...
        let th_3 = default_crypto().compute_th_3(&TH_2_TV, &plaintext_2_tv, &CRED_R_TV);
    ...
    fn test_compute_th_4() {
        ...
        let th_4 = default_crypto().compute_th_4(&TH_3_TV, &plaintext_3_tv, &CRED_I_TV);
    ...
    fn test_edhoc_kdf() {
        ...
        let output = default_crypto().edhoc_kdf(
            &PRK_2E_TV,
            0u8,
            &th_2_context_tv,
            SHA256_DIGEST_LEN,
        ...
        let output_2 = default_crypto().edhoc_kdf(
            &PRK_3E2M_TV,
            2u8,
            &context_info_mac_2,
    ...
    fn test_encrypt_message_3() {
        ...
        let message_3 = default_crypto().encrypt_message_3(&PRK_3E2M_TV, &TH_3_TV, &plaintext_3_tv);
    ...
    fn test_decrypt_message_3() {
        ...
        let plaintext_3 = default_crypto().decrypt_message_3(&PRK_3E2M_TV, &TH_3_TV, &message_3_tv);
    ...
    fn test_compute_mac_3() {
        let mac_3 =
            default_crypto().compute_mac_3(&PRK_4E3M_TV, &TH_3_TV, &ID_CRED_I_TV, &CRED_I_TV);
    ...
    fn test_compute_and_verify_mac_2() {
        let rcvd_mac_2 =
            default_crypto().compute_mac_2(&PRK_3E2M_TV, &ID_CRED_R_TV, &CRED_R_TV, &TH_2_TV);
    ...
    fn test_encrypt_decrypt_ciphertext_2() {
        ...
        // test decryption
        let (plaintext_2, plaintext_2_len) =
            default_crypto().encrypt_decrypt_ciphertext_2(&PRK_2E_TV, &TH_2_TV, &ciphertext_2_tv);
        ...
        // test encryption
        let (ciphertext_2, ciphertext_2_len) =
            default_crypto().encrypt_decrypt_ciphertext_2(&PRK_2E_TV, &TH_2_TV, &plaintext_2_tmp);
    ...
    fn test_compute_prk_4e3m() {
        let prk_4e3m = default_crypto().compute_prk_4e3m(&SALT_4E3M_TV, &SK_I_TV, &G_Y_TV);
        assert_eq!(prk_4e3m, PRK_4E3M_TV);
    }

    #[test]
    fn test_compute_prk_3e2m() {
        let prk_3e2m = default_crypto().compute_prk_3e2m(&SALT_3E2M_TV, &X_TV, &G_R_TV);
        assert_eq!(prk_3e2m, PRK_3E2M_TV);
    }

    #[test]
    fn test_compute_prk_2e() {
        let prk_2e = default_crypto().compute_prk_2e(&X_TV, &G_Y_TV, &TH_2_TV);
    ...
    fn test_compute_prk_out() {
        ...
        let prk_out_buf = default_crypto().edhoc_kdf(
            &PRK_4E3M_TV,
            7u8,
            &th_4_context,
            TH_4_TV.len(),
    ...
    fn test_compute_prk_exporter() {
        ...
        let prk_exporter_buf = default_crypto().edhoc_kdf(
            &PRK_OUT_TV,
            10u8,
            &[0x00; MAX_KDF_CONTEXT_LEN],
            0,
    ...
    fn test_compute_oscore_master_secret_salt() {
        let oscore_master_secret_buf = default_crypto().edhoc_kdf(
            &PRK_EXPORTER_TV,
            0u8,
            &[0x00; MAX_KDF_CONTEXT_LEN],
            0,
        ...
        let oscore_master_salt_buf = default_crypto().edhoc_kdf(
            &PRK_EXPORTER_TV,
            1u8,
            &[0x00; MAX_KDF_CONTEXT_LEN],
            0,
