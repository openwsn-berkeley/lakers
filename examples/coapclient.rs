...
fn main() {
    ...
    if verified {
        ...
        let (state, oscore_secret) = edhoc_exporter(state, U8(0), &BytesMaxContextBuffer::new(), 0, 16);
        let (state, oscore_salt) = edhoc_exporter(state, U8(1), &BytesMaxContextBuffer::new(), 0, 16);
